...
# Bastion VNIC Attachment DataSource
data "oci_core_vnic_attachments" "AppsBastionServer_VNIC1_attach" {
  availability_domain = var.availablity_domain_name == "" ? lookup(data.oci_identity_availability_domains.ADs.availability_domains[0], "name") : var.availablity_domain_name
  compartment_id      = oci_identity_compartment.AppsCompartment.id
  instance_id         = oci_core_instance.AppsBastionServer.id
}

# Bastion VNIC DataSource
data "oci_core_vnic" "AppsBastionServer_VNIC1" {
  vnic_id = data.oci_core_vnic_attachments.AppsBastionServer_VNIC1_attach.vnic_attachments.0.vnic_id
}
